...
$db=mysql_select_db("poifriuliveneziagiulia");
if(!$db){
   die ("Impossibile selezionare il database POI.".mysql_errno());
}

$query1="SELECT COMUNE,COD_PROVINCIA FROM parafarmacieprovinciatrento";
...
while ($row1=mysql_fetch_assoc($result1)){
	...
	$codiceprovincia=$row1["COD_PROVINCIA"];
	
	
	
	foreach($filedec["comuni"] as $value){
		$comune2=$value["nomeComune"];
		$codiceprovincia2=$value["codiceProvincia"];
		$provincia2=$value["Provincia"];
		$provincia2I=addslashes($provincia2);
		$idcomune=$value["id"];
		
		if($comune==$comune2 and $codiceprovincia==$codiceprovincia2){
		    
			
			$query="UPDATE farmacieprovinciatrento SET id_comune='$idcomune' WHERE COMUNE='$comuneI' and COD_PROVINCIA='$codiceprovincia'";
