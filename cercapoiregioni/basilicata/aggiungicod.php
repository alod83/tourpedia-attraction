...
$query1="SELECT Comune,Provincia FROM luoghicultura";
...
while ($row1=mysql_fetch_assoc($result1)){
	...
	while ($row2=mysql_fetch_assoc($result2)){
		...
		if(isset($row2["Denominazione_provincia"])){
			$provincia2=$row2["Denominazione_provincia"];
		}else if(isset($row2["Denominazione_Citta_metropolitana"])){
		...
		if($comune==$comune2 and $provincia==$provincia2){
		    $query="UPDATE  poiemiliaromagna2.luoghicultura SET codiceComune='$codcomune' WHERE Comune='$comuneI' and Provincia='$provinciaI'";
