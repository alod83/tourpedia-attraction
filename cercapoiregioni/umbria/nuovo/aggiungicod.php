...
$db=mysql_select_db("poiumbria2");
if(!$db){
   die ("Impossibile selezionare il database.".mysql_errno());
}

$query1="SELECT CITTA FROM ecomusei";
...
while ($row1=mysql_fetch_assoc($result1)){
	...
	if($comune=="Bastia"){
		$query="UPDATE poiumbria2.ecomusei SET codiceComune='54002' WHERE CITTA='$comuneI'";
		$do_query=mysql_query($query);
	}else if($comune=="Avigliano dell'Umbria"){
		$query="UPDATE poiumbria2.ecomusei SET codiceComune='55033' WHERE CITTA='$comuneI'";
		$do_query=mysql_query($query);
	}else if($comune=="Santa Maria degli Angeli"){
		$query="UPDATE poiumbria2.ecomusei SET codiceComune='54001' WHERE CITTA='$comuneI'";
		$do_query=mysql_query($query);
	}else if($comune=="Passignano su Trasimeno"){
		$query="UPDATE poiumbria2.ecomusei SET codiceComune='54038' WHERE CITTA='$comuneI'";
		$do_query=mysql_query($query);
	}
	while ($row2=mysql_fetch_assoc($result2)){
		...
		if($comune==$comune2 and ($provincia2=="PG" or $provincia2=="TR")){
		    $query="UPDATE poiumbria2.ecomusei SET codiceComune='$codcomune' WHERE CITTA='$comuneI'";
			$do_query=mysql_query($query);
			if(!$do_query){
